...
- **데이터 영속성**: JSON을 이용해 Sample/Order/ProductionJob 데이터를 저장/로드한다.
  ...
  (사용할 JSON 라이브러리는 별도 PoC에서 검증된 방식을 재사용 — 예: nlohmann/json)
...
- **supervisor** (`.claude/agents/supervisor.md`): `phase-planner`/`code-review`/`tester`가
  ...
  믿지 않고 파일을 직접 열어보거나 빌드·테스트를 다시 실행해 검증하며, 코드/문서를 직접
  수정하지 않는다. 전체 Phase가 끝난 뒤 한 번에 하는 최종 감사가 아니라 매 에이전트 호출
  직후 바로 수행한다.
