...
- [ ] `Model::Order`에 정적 팩토리 `static Order CreateReserved(std::string orderId, std::string sampleId, std::string customerName, int quantity, std::string createdAt)`
      ...
      한다).
- [ ] 주문 수량(`quantity`)이 1 이상의 정수인지 검증하는 로직은 `OrderService::PlaceOrder`
      내부(또는 `Order`의 별도 `static bool IsValidQuantity(int)` 헬퍼)에 둔다 — `Sample`의
      `IsValidYieldRate` 등과 동일한 패턴. **상한은 두지 않는다(확정).**
- [ ] 고객명(`customerName`)이 빈 문자열/공백만 있는 문자열이면 거부하는 검증도 함께 둔다
...
- [ ] `OrderService`(Model 계층 유스케이스 조립 클래스) 신설 — 아래 책임을 캡슐화해 Controller가
      도메인 규칙을 직접 알 필요 없게 함
  - [ ] `PlaceOrderResult PlaceOrder(const std::string& sampleId, const std::string& customerName, int quantity)`
    - [ ] `SampleRepository::FindById(sampleId)`로 등록된 시료인지 조회 (PRD 7.2 "등록된 시료만
    ...
    - [ ] 고객명이 빈 문자열/공백만 있는 문자열이면 마찬가지로 저장하지 않고 실패 반환
...
- [ ] `OrderController`(신설, `Controller::ISubMenuController`를 구현) — 주문 접수 메뉴 흐름
      담당
  - [ ] `void Run() override`: 주문 접수 하위 메뉴 루프(`SampleController::Run()`과 동일한
        패턴)
...
- [ ] `Controller::MainMenuController`를 확장해 두 번째 서브메뉴("2": 시료 주문)를 처리할 수
      있게 한다. 현재 생성자가 `ISubMenuController& sampleController` 하나만 받으므로, 다음 중
      하나를 택해 구현한다(둘 다 DIP를 유지하는 방법이며, 어느 쪽을 택할지는 구현자가 판단):
  - [ ] (A) 생성자에 `ISubMenuController& orderController` 인자를 추가하고 `HandleInput`에서
        `"2"` 분기를 새로 추가하는 최소 확장. 다만 Phase 3~6에서 서브메뉴가 계속 늘어나므로
        매 Phase마다 생성자 파라미터가 늘어나는 단점이 있다.
  - [ ] (B) 생성자를 `std::map<std::string, ISubMenuController&>`(또는 메뉴 키 → 컨트롤러
        레퍼런스 목록)를 받는 형태로 일반화해, 이후 Phase가 생성자 시그니처를 매번 바꾸지
        않고 항목만 추가하도록 한다.
  - [ ] 어느 쪽이든 기존 `SampleController` 배선("1")이 깨지지 않아야 하며, 관련 기존 테스트
        (`Tests/ControllerTests.cpp` 등)를 함께 갱신해야 한다.
- [ ] `main.cpp`에 `View::OrderView orderView;`, `Model::OrderService orderService(sampleRepository, orderRepository);`,
...
- [ ] 콘솔에서 메인 메뉴 → "2" 입력으로 시료 주문 메뉴 진입 → 시료 ID/고객명/수량 입력 →
...
- [ ] 생성된 주문이 JSON 파일(`data/orders.json`)에 영속화되며, 앱 재시작(재로드) 후에도
      동일하게 조회된다.
- [ ] `main.cpp`가 `OrderService`/`OrderView`/`OrderController`를 생성해 배선하고, 기존
...
- [ ] `tester` 에이전트가 위 7장의 테스트 시나리오를 GoogleTest/GMock으로 작성하고,
      `SampleOrderSystem.exe --test`(Debug) 빌드 및 테스트 실행이 전부 통과한다.
- [ ] Phase 3이 그대로 이어받을 수 있도록 `OrderRepository::FindAll()`을 통한 `RESERVED`
